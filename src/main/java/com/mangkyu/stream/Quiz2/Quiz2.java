...
public class Quiz2 {
    ...
    public Map<String, Integer> quiz1() {
        ...
        return WORDS.stream().map(word -> word.substring(0,1))
    ...
    public String quiz2() {
        ...
        return WORDS.stream().filter(word -> word.length() > 1).map(word -> word.substring(0,1).toUpperCase()).collect(Collectors.joining(" "));
    }
